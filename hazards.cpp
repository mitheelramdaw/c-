...
// Capitalise first letter of racer name
...
int main() {
    ...
    while (!raceHasFinished) {
        ...
        if (racerOneLaps >= maxLaps && racerTwoLaps >= maxLaps) {
            cout << "\nResult: It's a TIE! \xf0\x9f\x8f\x81\n";
            raceHasFinished = true;
        } else if (racerOneLaps >= maxLaps) {
            cout << "\nResult: " << colour(racerOneName, "32;1") << " wins! \xf0\x9f\x8f\x86\n";
            raceHasFinished = true;
            ofstream lb("race_leaderboard.txt", ios::app);
            if (lb) lb << racerOneName << " beat " << racerTwoName << " in " << turns << " turns (code " << seed << ")\n";
        } else if (racerTwoLaps >= maxLaps) {
            cout << "\nResult: " << colour(racerTwoName, "36;1") << " wins! \xf0\x9f\x8f\x86\n";
            raceHasFinished = true;
            ofstream lb("race_leaderboard.txt", ios::app);
            if (lb) lb << racerTwoName << " beat " << racerOneName << " in " << turns << " turns (code " << seed << ")\n";
        }
